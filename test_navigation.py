...
class NavigationTests(unittest.TestCase):
    ...
    def test_primary_navigation_and_utilities_stay_out_of_creation_path(self):
        for page in PAGES:
            ...
            self.assertNotIn('재료 찾기', nav)
            self.assertIn('재료 검수', nav)
        ...
        for marker in ('id="savePlan"', 'class="file-tools"'):
            self.assertGreater(text.index(marker), tools)
        self.assertNotIn('id="mapWorkbench"', text)
        self.assertNotIn('id="materialsDetails"', text)
        self.assertIn('id="branchGroups"', text)
        ...
        self.assertNotIn('class="home-archive"', home)
        self.assertNotIn('seminar-structure-report', home)
