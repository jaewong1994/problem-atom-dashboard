...
function showSavedSelection(){
  ...
  try{
    const current=JSON.parse(localStorage.getItem(window.PAAccount?.storageKey('pa-problem-design-v1')||'pa-problem-design-v1')||'null');
    if(current?.schema==='problem-atom/design-draft/1'){action.textContent='이어서 문항 만들기';note.textContent='핵심 조건 '+current.design.elements.length+'개 · 추론 갈래 '+current.design.branches.length+'개가 저장돼 있어요.';return;}
...
(function(){const host=document.getElementById('homeUnits');if(!host||typeof PACurriculum==='undefined')return;const C=PACurriculum;
 ...
 fetch('connection-registry.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error();return r.json();}).then(registry=>{for(const node of host.querySelectorAll('[data-unit-count]')){const count=C.count(registry,node.dataset.unitCount);node.textContent='자산 '+count+'개';if(!count){const link=node.closest('a');link.removeAttribute('href');link.setAttribute('aria-disabled','true');link.title='분석 재료가 준비되면 제작할 수 있습니다.';}}}).catch(()=>{});
})();
