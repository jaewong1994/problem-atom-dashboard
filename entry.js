...
(function(){const host=document.getElementById('homeUnits');if(!host||typeof PACurriculum==='undefined')return;const C=PACurriculum;
 ...
 fetch('connection-registry.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error();return r.json();}).then(registry=>{for(const node of host.querySelectorAll('[data-unit-count]')){const count=C.count(registry,node.dataset.unitCount);node.textContent='자산 '+count+'개';}}).catch(()=>{});
})();
