...
1. Choose your name on the home screen. On the first login, enter the one-time invitation and set your own password (at least 12 characters).
...
5. Add an account by name and deliver the displayed invitation privately. New accounts are teachers, not administrators. Invitation renewal invalidates the old code. No teacher password is visible to administrators.
6. Delete disables account access immediately, including existing JWTs at the application boundary. Analysis, comments and reviews remain. Restore recovers the same identity; pending accounts need a new invitation. Self-deletion is blocked.
...
- Account changes are serialized and audited. Invitations are 256-bit random values stored only as SHA-256 hashes, expiring after seven days. Initial setup has a two-minute lease and can recover an Auth user after an interrupted activation.
...
1. Run the normal `prepare_pages.py --build` (100 tests currently).
2. `node supabase/prepare-deploy.cjs` prepares schema, catalog/question mappings and a single-file Edge bundle in ignored `.pa-team/supabase-deploy`. Initial teacher invitations are generated there **once** and never published. Treat this folder as private.
3. Apply `supabase/team.sql` and the prepared `seed.sql` in the named project. Existing claims import once; later seed runs update only the catalog/question mappings.
