(function(root,factory){const api=factory(typeof module==='object'?require('./model-contract.js'):root.PAModelContract);if(typeof module==='object')module.exports=api;else root.PAWebHandoff=api;})(typeof globalThis!=='undefined'?globalThis:this,function(C){
 ...
 function prompt(request,provider='gemini'){
  ...
  const ids=new Set(request.seed_plan.nodes.map(n=>n.id)),ops=request.design_intent?.mode==='problem_design'?request.knowledge.operations:request.knowledge.operations.filter(o=>ids.has(o.id));
  ...
  const payload={schema:request.schema,request_id:request.request_id,registry_revision:request.registry_revision,brief:request.brief,design_intent:request.design_intent,seed_plan:request.seed_plan,
   knowledge:{operations:ops,types:Object.fromEntries(Object.entries(request.knowledge.types).filter(([k])=>types.has(k))),rules:request.knowledge.rules,language:request.knowledge.language,curriculum:request.knowledge.curriculum,authoring:request.knowledge.authoring,problem_design:request.knowledge.problem_design,reviewed_assets:request.knowledge.reviewed_assets||[],composition_recipes:request.knowledge.composition_recipes||[]},response_schema:request.response_schema};
  return `고등학교 수학 문항 한 개를 제작해 주세요. 아래 제작 조건과 대단원 범위를 지키세요. 사용 중인 AI로 답하고 다른 모델을 호출하지 마세요.\n${C.INSTRUCTIONS}\n조건 중심 요청에는 선택 범위에서 쓸 수 있는 등록 재료를 담았습니다. 기존 경로 요청에는 선택한 재료만 담았습니다. 다른 원자가 꼭 필요하면 unresolved에 이유를 적고 임의 ID를 만들지 마세요.\n답변은 response_schema에 맞는 JSON 객체 하나만 출력하세요. JSON 문자열 속 LaTeX의 역슬래시는 반드시 두 번 쓰세요. 질문과 해설은 문단별 문자열 배열로 작성하세요. 그림 없이 이해할 수 있는 텍스트·수식 문항으로 작성하세요.\n\n${JSON.stringify(payload,null,2)}`;
 }
 function parse(text){
