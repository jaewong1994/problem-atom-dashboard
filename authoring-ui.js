(function(root){'use strict';
 ...
 function mount(host,{example}){
  let ctx,selected=null,expandedGoals=false,lastIds=new Set(),hasRendered=false,arrivalIds=new Set(),arrivalUntil=0,drawFrame,libraryMode="bundles",libraryReviewed=false,libraryTerm="",libraryLimit=8,lastScope="";const expandedBundles=new Set(),openExamples=new Set();
  ...
  function render(context){
   ...
   function reviewContext(panel,ids){
    const rows=(registry.reviewed_assets||[]).filter(a=>a.operation_ids.some(id=>ids.includes(id)));
   ...
   if(selected==='@library'){
    ...
    const paint=()=>{shelf.replaceChildren();const term=libraryTerm.trim().toLowerCase(),rows=available[libraryMode].filter(b=>(!libraryReviewed||(B.definition(b.id)?.members||[b.id]).every(id=>PAReviewModel.forOperation(registry,id).length))&&[b.name,b.searchAlias,b.idea,b.example,C.atoms[b.id]?.name,...Object.values(X.get(b.id)||{}).flat()].join(' ').toLowerCase().includes(term)).sort((a,b)=>Number(U.material(registry,B.definition(b.id)?.members||[b.id],ctx.unitScope).main)-Number(U.material(registry,B.definition(a.id)?.members||[a.id],ctx.unitScope).main));
     for(const b of rows.slice(0,libraryLimit)){
      ...
      top.append(el('span',pack?'판단 묶음 · '+b.members.length+'개':'낱개 재료','author-choice-meta'),el('span','⌄','box-chevron'));top.lastElementChild.setAttribute('aria-hidden','true');title.textContent=b.name;summary.append(top,title);
    ...
    const p=A.profile(selected);heading('선택한 풀이',name(selected));reviewContext(panel,[selected]);const parent=layout.bundles.find(b=>b.members.includes(selected));if(parent)panel.append(button('← '+parent.name,()=>focusNode('@bundle:'+parent.id),'text-button'));
