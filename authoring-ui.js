(function(root){'use strict';
 const A=root.PAAuthoring,C=root.PABoxCopy,G=root.PAGraph,B=root.PABundles;
 ...
 function mount(host,{example}){
  let ctx,selected=null,expandedGoals=false,lastIds=new Set(),hasRendered=false,arrivalIds=new Set(),arrivalUntil=0,drawFrame,libraryMode="bundles",libraryTerm="",libraryLimit=8;const expandedBundles=new Set();
  ...
  function render(context){
   ctx=context;const {plan,registry,coreId,target,audit,choices,actions}=ctx,layout=B.view(plan,registry,[...expandedBundles]),graph=layout.graph;host.classList.toggle('is-empty',!plan.nodes.length);
   ...
   bar.replaceChildren();const title=el('div');title.append(el('span','DESIGN CANVAS','author-kicker'),el('h2',plan.nodes.length?'판단을 묶어 문항 만들기':'함께 쓰는 생각을 한 묶음으로'));bar.append(title,button('재료 넣기',()=>focusNode('@library'),'author-change'));
   ...
   if(selected==='@library'){
    heading('재료 고르기','묶어서 넣거나, 하나부터 시작하세요');panel.append(el('p','여러 판단이 이어지는 재료는 묶음으로 담습니다. 예시를 보고 고른 뒤 안쪽 판단을 펼쳐볼 수 있어요.','author-intro'));
    ...
    panel.append(el('p','현재 연결 재료는 수학Ⅱ·미적분Ⅰ 중심입니다. 묶음은 과목 고정 틀이 아니며, 새 과목의 판단과 사용 조건도 같은 방식으로 등록합니다.','author-footnote'));
    const shelf=el('div',null,'bundle-shelf');panel.append(shelf);
    const paint=()=>{shelf.replaceChildren();const term=libraryTerm.trim().toLowerCase(),rows=(libraryMode==='bundles'?B.catalog:registry.operations.map(o=>({id:o.id,name:name(o.id),idea:A.profile(o.id)?.decision||o.name,example:C.atoms[o.id]?.example||''}))).filter(b=>[b.name,b.idea,b.example].join(' ').toLowerCase().includes(term));
     for(const b of rows.slice(0,libraryLimit)){const pack=B.definition(b.id),card=button('',()=>focusNode('@preview:'+b.id),'bundle-library-card');card.dataset.bundleChoice=b.id;card.append(el('span',pack?'묶음 · '+b.members.length+'개 재료':'낱개 재료','author-choice-meta'),el('b',b.name),el('small',b.idea),el('span','예시와 사용 조건 보기 →','bundle-open'));shelf.append(card);}
     if(rows.length>libraryLimit)shelf.append(button('재료 '+Math.min(8,rows.length-libraryLimit)+'개 더 보기',()=>{libraryLimit+=8;paint();},'wide'));if(!rows.length)shelf.append(el('p','맞는 재료가 없습니다. 다른 말로 찾아보세요.','author-empty'));
    };search.oninput=()=>{libraryTerm=search.value;libraryLimit=8;paint();};paint();
   }else if(selected.startsWith('@preview:')){
    const id=selected.slice(9),b=B.definition(id),startOptions=A.startOptions(registry,id),seed=startOptions.seed,p=A.profile(id),placements=plan.nodes.length?B.placements(plan,registry,id):[];
    heading(b?'판단 묶음 · '+b.members.length+'개 재료':'낱개 재료',b?b.name:name(id));panel.append(button('← 재료 목록',()=>focusNode('@library'),'text-button'));
    if(b){panel.append(paragraph('문제에 더해지는 성격',b.effect,'spotlight'),el('p',b.example,'bundle-example'));const list=el('ol',null,'bundle-members');for(const member of b.members){const li=el('li');li.append(el('b',name(member)),el('p',A.profile(member)?.decision));list.append(li);}panel.append(list);}else panel.append(paragraph('여기서 떠올릴 생각',p?.decision||name(id),'spotlight'),example(C.atoms[id]?.example));
    const conditions=el('details',null,'author-inputs');conditions.append(el('summary','필요한 시작 정보 '+seed.plan.facts.length+'개 · 사용 조건'));for(const fact of seed.plan.facts)conditions.append(el('p',registry.types[fact.type]));for(const n of seed.plan.nodes){const op=registry.operations.find(o=>o.id===n.id);conditions.append(el('p',op.guard_note,'author-footnote'));}panel.append(conditions,el('p','현재 구현 과목: '+(B.courses(registry,id).join(' · ')||'과목 사이 연결 검토 필요')+' · 검토 전 자산','author-footnote'));
    if(!startOptions.hasQuestion){panel.append(el('p','이 낱개는 풀이 중간에 이어 쓰는 재료입니다. 혼자서는 마지막 질문까지 이어지지 않아요.','author-caution'));for(const pack of startOptions.bundles)panel.append(button('함께 쓰는 묶음: '+pack.name,()=>focusNode('@preview:'+pack.id),'wide'));}
    if(!plan.nodes.length&&startOptions.hasQuestion){const start=button(b?'이 묶음 담기':'이 재료 담기',()=>{actions.startMaterial(id);focusNode('@question');},'primary wide');start.dataset.bundleStart=id;panel.append(start,el('p','필요한 시작 정보도 함께 준비합니다. 다음 화면에서 확인·수정할 수 있어요.','author-footnote'));}
     ...
     if(startOptions.hasQuestion){const change=el('details');change.append(el('summary','이 재료로 새 구성 시작'),el('p','현재 재료를 바꾸고 필요한 시작 정보를 준비합니다. 되돌리기로 복구할 수 있어요.','author-footnote'),button('현재 구성을 바꾸고 시작',()=>{actions.startMaterial(id);focusNode('@question');},'wide'));panel.append(change);}
    }
   }else if(selected.startsWith('@bundle:')){
    const b=layout.bundles.find(b=>'@bundle:'+b.id===selected);heading('판단 묶음 · '+b.members.length+'개 재료',b.name);panel.append(paragraph('이 묶음이 하는 일',b.effect,'spotlight'),el('p',b.example,'bundle-example'));
