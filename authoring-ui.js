 ...
 function mount(host,{example}){
  ...
  function render(context){
   ...
   if(selected==='@library'){
    ...
    const id=selected.slice(9),b=B.definition(id),seed=B.seed(registry,id),p=A.profile(id),placements=plan.nodes.length?B.placements(plan,registry,id):[];
    ...
    if(!plan.nodes.length){const start=button(b?'이 묶음 담기':'이 재료 담기',()=>{actions.startMaterial(id);focusNode('@question');},'primary wide');start.dataset.bundleStart=id;panel.append(start,el('p','필요한 시작 정보도 함께 준비합니다. 다음 화면에서 확인·수정할 수 있어요.','author-footnote'));}
    else{for(const item of placements.slice(0,3)){panel.append(button('현재 풀이에 '+item.nodes.length+'개 이어 붙이기',()=>{actions.addMaterial(item);focusNode(b?'@bundle:'+id:id);},'primary wide'));}
     ...
     const change=el('details');change.append(el('summary','이 재료로 새 구성 시작'),el('p','현재 재료를 바꾸고 필요한 시작 정보를 준비합니다. 되돌리기로 복구할 수 있어요.','author-footnote'),button('현재 구성을 바꾸고 시작',()=>{actions.startMaterial(id);focusNode('@question');},'wide'));panel.append(change);
    }
