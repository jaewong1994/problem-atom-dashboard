(function(){'use strict';
 ...
 async function admin(){
  ...
  dialog.append(header,node('p','이름으로 계정을 추가하세요. 당사자가 첫 로그인 때 초대 코드 000000을 입력하고 비밀번호를 설정합니다.'),form,issued,status,list,node('small','계정을 삭제하면 로그인이 차단됩니다. 기존 분석·검수 기록은 보존되며 여기서 계정을 복원할 수 있습니다.'));document.body.append(dialog);dialog.showModal();
  ...
  async function run(body,teacher){status.textContent='처리 중…';try{const result=await call('admin',body);render(result.members);status.textContent=body.action==='list'?'':teacher+' · 처리했습니다.';
   if(result.invite){issued.hidden=false;issued.replaceChildren(node('h3',teacher+' 강사 초대 코드'),node('p','최초 초대 코드는 000000입니다. 비밀번호를 설정하기 전까지 사용할 수 있습니다.'));const value=node('textarea');value.readOnly=true;value.value=result.invite;value.setAttribute('aria-label',teacher+' 초대 코드');const copy=node('button','초대 코드 복사');copy.type='button';copy.onclick=async()=>{try{await navigator.clipboard.writeText(result.invite);copy.textContent='복사됨';}catch(_){value.select();status.textContent='선택된 코드를 복사해 주세요.';}};issued.append(value,copy);}
   return result;
 ...
 async function entry(){
  ...
  host.append(node('h1','강사 계정으로 시작하기'),node('p','처음이면 초대 코드 000000을 입력하고 비밀번호를 설정하세요. 기존 분석 기록은 계정에 연결됩니다.'));
  ...
  const setup=node('div',null,'team-setup'),inviteLabel=node('label','최초 설정용 초대 코드'),invite=node('input');invite.type='text';invite.inputMode='numeric';invite.placeholder='000000';invite.autocomplete='off';invite.maxLength=128;inviteLabel.append(invite);
  const confirmLabel=node('label','비밀번호 다시 입력'),confirm=node('input');confirm.type='password';confirm.autocomplete='new-password';confirm.maxLength=256;confirmLabel.append(confirm);setup.append(inviteLabel,confirmLabel,node('small','초대 코드는 000000입니다. 비밀번호는 6자 이상으로 정해 주세요.'));setup.hidden=true;
  const submit=node('button','로그인','team-primary');submit.type='submit';name.onchange=()=>{const first=members.find(m=>m.name===name.value)?.needsSetup;setup.hidden=!first;invite.required=confirm.required=Boolean(first);password.autocomplete=first?'new-password':'current-password';password.minLength=first?6:1;submit.textContent=first?'비밀번호 설정하고 시작':'로그인';status.textContent='';};
  form.append(label,pwLabel,setup,submit,status);host.append(form);
  form.onsubmit=async event=>{event.preventDefault();if(!setup.hidden&&password.value!==confirm.value){status.textContent='두 비밀번호가 일치하지 않습니다.';return;}submit.disabled=true;status.textContent='로그인 확인 중…';try{current=await call('login',{name:name.value,password:password.value,...(!setup.hidden?{invite:invite.value.trim()}:{})});password.value=confirm.value=invite.value='';identity();bar();await entry();}catch(e){status.textContent=e.message;}finally{submit.disabled=false;}};
 }
