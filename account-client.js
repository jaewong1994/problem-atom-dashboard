(function(){'use strict';
 ...
 function bar(){let el=document.getElementById('teamAccountBar');if(!el){el=node('aside',null,'team-account-bar');el.id='teamAccountBar';document.querySelector('.global-header').after(el);}el.replaceChildren();if(!current?.user){el.append(node('span','강사 계정으로 로그인해 주세요.'));return;}el.append(node('strong',current.user.name+' 강사'));if(current.mode)el.append(node('span',current.mode==='codex'?'Codex 구독으로 제작':'AI 웹으로 제작'));const logout=node('button','로그아웃');logout.type='button';logout.onclick=async()=>{try{await call('logout',{});location.assign('index.html');}catch(e){el.append(node('span',e.message));}};el.append(logout);}
 ...
 const ready=enabled?(async()=>{try{current=await call('me');identity();bar();await entry();return current;}catch(e){const main=document.getElementById('main');if(main){main.classList.add('team-locked');const box=node('section',null,'team-entry');box.id='teamEntry';box.append(node('h1','계정 서버 연결 확인'),node('p',e.message));main.prepend(box);}throw e;}})():Promise.resolve(null);
