(function(root,factory){const api=typeof module==='object'?factory(require('./connection-engine.js'),require('./selection-model.js')):factory(root.PAConnections,root.PASelection);if(typeof module==='object')module.exports=api;else root.PAPlanner=api;})(typeof globalThis!=='undefined'?globalThis:this,(Connections,Selection)=>{
 'use strict';
 ...
 function normalizeIntent(value,plan,registry){
  ...
  return {schema:value.schema,core:clone(core),target:{type:t.type,subject:t.subject,scope:t.scope,...(relation?{object:t.object}:{})},calculation:value.calculation,reasoning:value.reasoning};
 }
 function intent(plan,registry,coreId,target,calculation,reasoning){return normalizeIntent({schema:'problem-atom/design-intent/1',core:plan.nodes.find(n=>n.id===coreId),target,calculation,reasoning},plan,registry);}
